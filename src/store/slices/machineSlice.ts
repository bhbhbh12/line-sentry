...
const machineSlice = createSlice({
  ...
  reducers: {
    ...
    updateMachinePositionLocal: (state, action) => {
      const { machineId, position, dimensions } = action.payload;
      const machine = state.machines.find(m => m._id === machineId);
      if (machine) {
...
export const { clearError, updateMachineStatus, updateMachinePositionLocal } = machineSlice.actions;
